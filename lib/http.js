...
SuperagentHttpClient.prototype.execute = function (obj) {
  ...
  if (!isNaN(obj.maxRedirects)) {
    r.redirects(obj.maxRedirects);
  }
  ...
  if(obj.body) {
    if(_.isObject(obj.body)) {
      ...
      if (contentType.indexOf('multipart/form-data') === 0) {
        ...
        var parameters = obj.operation.parameters || []
        var formParams = parameters.filter(p => p.in === 'formData')
        formParams.forEach(({name, type}) => {
          var value = obj.body[name]
          if(type === 'file') {
            // assume value is 'multer' (https://www.npmjs.com/package/multer) file info format
            r.attach(name, value.path, {contentType: value.mimetype, filename: value.originalname})
          }
          else if(Array.isArray(value)) {
            value.forEach(v => r.field(name, v))
          }
          else {
            r.field(name, value);
          }
