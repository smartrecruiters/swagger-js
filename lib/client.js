  ...
  var obj = {
    ...
    on: {
      ...
      response: function (resp) {
        ...
        self.specification = _.cloneDeep(responseObj)
        ...
        if (responseObj.swagger && parseInt(responseObj.swagger) === 2) {
          ...
          self.oldSwaggerResources = converter.oldSwaggerResources

          converter.setDocumentationLocation(self.url);
          converter.convert(responseObj, self.clientAuthorizations, self.options, function(spec) {
            self.swaggerObject = spec;
            new Resolver(self.logger).resolve(spec, self.url, self.buildFromSpec, self);
            self.isValid = true;
          });
        }
      }
